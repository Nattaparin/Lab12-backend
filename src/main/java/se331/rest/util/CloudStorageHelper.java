...
public class CloudStorageHelper {
    ...
    static {
        ...
        try {
            serviceAccount = new ClassPathResource("imageupload-bdda5-2f53defd8649.json").getInputStream();
            storage = StorageOptions.newBuilder().setCredentials(GoogleCredentials.fromStream(serviceAccount)).setProjectId("imageupload-bdda5").build().getService();
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public  String getImageUrl(MultipartFile file, final String bucket) throws IOException, ServerException{
        ...
        if(fileName != null && !fileName.isEmpty() && fileName.contains(".")){
            ...
            throw  new ServerException("file must be an image");
        }
        return null;
    }
}
